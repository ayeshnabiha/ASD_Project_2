...
namespace {
bool markAllPassedReservationsCompleted(sqlite3* db) {
        ...
        WHERE status != 'Cancelled'
          ...
          AND datetime(
                printf('%04d-%02d-%02d %02d:%02d:00',
                    ...
                    time_stop_hour, time_stop_minutes
                )
    ...
    if (rc != SQLITE_OK) {
        std::cerr << "Failed to prepare bulk completed update statement: " << sqlite3_errmsg(db) << std::endl;
        return false;
    }
...
Node* loadReservations(sqlite3* db) {
    if (!markAllPassedReservationsCompleted(db)) {
        std::cerr << "Warning: failed to bulk-update passed reservations to Completed." << std::endl;
    }
